...
const Judge = () => {
  ...
  useEffect(() => {
    ...
    setInvitations(judgeInvitations)
  }, [navigate])
  ...
  const handleAcceptInvitation = (invitation) => {
    ...
    localStorage.setItem('judge_invitations', JSON.stringify(updatedInvitations))

    // Send notification to organizer (would be API call in production)
    const notifications = JSON.parse(localStorage.getItem('notifications') || '[]')
    notifications.push({
      id: Date.now(),
      type: 'judge_accepted',
      message: `${userData.name} accepted your judge invitation for ${invitation.hackathonName}`,
      timestamp: new Date().toISOString(),
      read: false
    })
    localStorage.setItem('notifications', JSON.stringify(notifications))

    alert(`✅ You accepted the invitation to judge ${invitation.hackathonName}!`)
  }
  ...
  const handleDeclineInvitation = (invitation) => {
    ...
    notifications.push({
      id: Date.now(),
      type: 'judge_declined',
      message: `${userData.name} declined your judge invitation for ${invitation.hackathonName}`,
      timestamp: new Date().toISOString(),
      read: false
    })
    localStorage.setItem('notifications', JSON.stringify(notifications))

    alert(`You declined the invitation to judge ${invitation.hackathonName}`)
  }
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      {invitations.filter(inv => inv.status === 'pending').length > 0 && (
        <div className="bg-gradient-to-r from-purple-600/10 to-pink-600/10 border border-purple-500/20 rounded-xl p-6 mb-6">
          ...
          <div className="space-y-4">
            {invitations.filter(inv => inv.status === 'pending').map(invitation => (
              <div
                key={invitation.id}
                className="bg-slate-900/50 border border-slate-700/50 rounded-lg p-4 hover:border-purple-500/50 transition"
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <h3 className="text-lg font-semibold text-white mb-2">{invitation.hackathonName}</h3>
                    <div className="grid grid-cols-2 gap-3 text-sm mb-4">
                      <div>
                        <span className="text-slate-400">Category:</span>
                        <span className="text-white ml-2">{invitation.hackathonDetails.category}</span>
                      </div>
                      <div>
                        <span className="text-slate-400">Mode:</span>
                        <span className="text-white ml-2">{invitation.hackathonDetails.mode}</span>
                      </div>
                      <div>
                        <span className="text-slate-400">Duration:</span>
                        <span className="text-white ml-2">
                          {new Date(invitation.hackathonDetails.startDate).toLocaleDateString()} - {new Date(invitation.hackathonDetails.endDate).toLocaleDateString()}
                        </span>
                      </div>
                      <div>
                        <span className="text-slate-400">Rounds:</span>
                        <span className="text-white ml-2">{invitation.hackathonDetails.rounds}</span>
                      </div>
                    </div>
